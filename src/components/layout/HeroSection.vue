<template>
  <section class="py-12">
    <div class="mb-10 text-center">
      <h2 class="text-4xl font-bold text-black dark:text-white">Highlights</h2>
    </div>
    ...
    <div class="max-w-7xl mx-auto">
        ...
        >
          ...
          >
            <div class="relative w-full h-[500px] rounded-lg overflow-hidden">
              ...
              <!-- Product Name -->
              <div class="absolute top-2 left-2 bg-black bg-opacity-20 text-white px-2 py-1 rounded-3xl text-sm font-semibold">
                {{ product.name }}
              </div>

              <!-- Price & Stock -->
              <div class="absolute bottom-10 left-2 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-sm">
            ...
            <div class="flex justify-end mt-2">
              <button class="flex p-2 bg-black dark:bg-white text-white dark:text-black max-h-8 max-w-8 justify-center items-center rounded-full">
      ...
      <div class="flex justify-center mt-4 space-x-4">
        <button @click="prevSlide" class="text-black dark:text-white">
          <Icon icon="mdi:chevron-left" class="w-6 h-6"/>
        </button>
        <button @click="nextSlide" class="text-black dark:text-white">
          <Icon icon="mdi:chevron-right" class="w-6 h-6"/>
        </button>
      </div>
    </div>
...
// Normalize images: tek string -> array
const productList = products.map((p) => ({
  ...p,
  images: Array.isArray(p.image) ? p.image : [p.image]
}));

// Active image index for each product
const activeImageIndexes = ref(productList.map(() => 0));
...
const nextSlide = () => {
  if (currentIndex.value < productList.length - visibleItems.value) currentIndex.value++;
};
const prevSlide = () => {
...
const nextImage = (i) => {
  if (activeImageIndexes.value[i] < productList[i].images.length - 1) activeImageIndexes.value[i]++;
};
const prevImage = (i) => {
